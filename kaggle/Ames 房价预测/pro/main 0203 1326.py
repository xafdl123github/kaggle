...
'''相关性高的特征'''
numerical_features = ['LotFrontage','LotArea','YearBuilt', 'MasVnrArea', 'TotalBsmtSF','1stFlrSF','LowQualFinSF','GrLivArea','WoodDeckSF','PoolArea']
non_numerical_features = ['Foundation','OpenPorchSF','OverallCond','TotRmsAbvGrd','BsmtExposure','Condition1','Functional','BsmtCond','BsmtFinSF1','GarageQual','SaleCondition','CentralAir','Fireplaces','MasVnrType','BsmtFullBath','Exterior1st','BldgType','2ndFlrSF','OverallQual','FullBath','MSZoning','FireplaceQu','MiscVal','Neighborhood','BsmtFinType1','Heating','PavedDrive','Utilities','Fence','GarageType','MSSubClass','KitchenAbvGr','LotConfig','HouseStyle','RoofMatl','SaleType','Street','ExterQual','SalePrice_Log','Electrical','GarageCars','KitchenQual','LandContour','HeatingQC']
...
# 选取的特征
new_features = numerical_features + non_numerical_features
...
pipe_best_gbr = Pipeline([
    ('pca', PCA(n_components=20)),
    ('std_scaler', RobustScaler()),
    ('gbr', GradientBoostingRegressor(random_state=2, n_estimators=850, learning_rate=0.03, max_depth=12, max_features=0.5, min_samples_leaf=45, loss='huber', alpha=0.95, ))
])
...
# ])
# param_grid = {
#     'pca__n_components': [16,18,20,22,24],
...
submission.to_csv('./submit/sub27.csv', index=False)