...
pipe_best_lasso = Pipeline([
    ('std_scaler', RobustScaler()),
    ('lasso', Lasso(random_state=1, alpha=0.0007, selection='random', tol=0.0001))
])
...
pipe_best_en = Pipeline([
    ('std_scaler', RobustScaler()),
    ('en', ElasticNet(random_state=1, alpha=0.00075, l1_ratio=0.9, selection='cyclic', tol=5e-05))
])
...
pipe_best_svr = Pipeline([
    ('std_scaler', RobustScaler()),
    ('svr', SVR(C=56, gamma=0.00025, kernel='rbf', epsilon=0.018, tol=0.01))
])
...
submission.to_csv('./submit/sub14.csv', index=False)