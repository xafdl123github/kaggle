...
"""
ic| X_train_dumm.shape: (1458, 183)
ic| X_test_dumm.shape: (1459, 183)
"""
...
# ])
# param_grid = {
#     'lasso__alpha': [0.001,0.003,0.0009],
...
pipe_best_lasso = Pipeline([
    ('std_scaler', RobustScaler()),
    ('lasso', Lasso(random_state=1, alpha=0.0009, selection='random', tol=0.0001))
])
...
pipe_best_en = Pipeline([
    ('std_scaler', RobustScaler()),
    ('en', ElasticNet(random_state=1, alpha=0.00095, l1_ratio=0.8, selection='cyclic', tol=5e-05))
])
...
pipe_best_svr = Pipeline([
    ('std_scaler', RobustScaler()),
    ('svr', SVR(C=55, gamma=0.00015, kernel='rbf', epsilon=0.018, tol=0.01))
])
...
submission.to_csv('./submit/sub15.csv', index=False)