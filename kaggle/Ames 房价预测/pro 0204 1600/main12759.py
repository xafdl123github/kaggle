...
'''相关性高的特征'''
numerical_features = ['LotFrontage','LotArea','YearBuilt', 'MasVnrArea','TotalSF','LowQualFinSF','GrLivArea','WoodDeckSF','PoolArea','TotalPorch','MiscVal']

shuzhi_features = ['OverallAssess','TotRmsAbvGrd','Fireplaces','FullBath','MSSubClass','KitchenAbvGr']

non_numerical_features = ['Foundation','BsmtExposure','Condition1','Functional','BsmtCond','BsmtFinSF1','GarageQual','SaleCondition','CentralAir','MasVnrType','BsmtFullBath','Exterior1st','BldgType','MSZoning','FireplaceQu','Neighborhood','BsmtFinType1','Heating','PavedDrive','Fence','GarageType','LotConfig','HouseStyle','RoofMatl','SaleType','Street','ExterQual','SalePrice_Log','Electrical','GarageCars','KitchenQual','LandContour','HeatingQC']
...
pipe_best_lasso = make_pipeline(RobustScaler(), Lasso(alpha=0.0005, random_state=1))
'''交叉验证'''
...
pipe_best_gbr = Pipeline([
    ('gbr', GradientBoostingRegressor(random_state=2, n_estimators=1000, learning_rate=0.03, max_depth=4, max_features=0.3, min_samples_leaf=30, loss='huber', alpha=0.95, ))
])
...
#     'svr__gamma': ['scale'],
#     'svr__C': [1.6],
#     'svr__kernel': ['linear'],  # linear 特别花费时间
#     'svr__tol': [1e-2],
#     'svr__epsilon': [0.09]
# }
# gsearch = GridSearchCV(estimator=pipe, param_grid=param_grid, cv=5, scoring='neg_mean_squared_error', n_jobs=-1, verbose=2)
...
pipe_best_svr = Pipeline([
    ('svr', SVR(C=1.6, gamma='scale', kernel='linear', epsilon=0.09, tol=0.01))
])
...
'''训练'''
pipe_best_lasso.fit(X_train_dumm, y)
y_pred = pipe_best_lasso.predict(X_test_dumm)
