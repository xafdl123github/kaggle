...
# GradientBoostingRegressor 算法  kaggle得分：0.15578
# pipe = Pipeline([
#     ('std_scaler', StandardScaler()),
...
#     'gbr__learning_rate': [0.01, 0.03, 0.05, 0.09, 0.1, 0.12, 0.14, 0.16, 0.18],
#     'gbr__max_depth': list(range(2, 40, 2)),
#     'gbr__max_features': [0.3, 0.5, 0.7],
#     'gbr__min_samples_leaf': [50, 70, 90, 110, 130]
# }
...
pipe_best_gbr = Pipeline([
    ('std_scaler', StandardScaler()),
    ('gbr', GradientBoostingRegressor(random_state=2, n_estimators=300, learning_rate=0.09, max_depth=2, max_features=0.7, min_samples_leaf=50))
])


# SVR 算法  kaggle得分:0.14011
# pipe = Pipeline([
#     ('std_scaler', StandardScaler()),
#     ('svr', SVR())
# ])
...
pipe_best_svr = Pipeline([
    ('std_scaler', StandardScaler()),
    ('svr', SVR(C=1e6, gamma=0.01, kernel='rbf'))
])
...
voting_reg.fit(X_train_reduction, y)
y_pred = voting_reg.predict(X_test_dumm_reduction)
'''生成csv文件'''
