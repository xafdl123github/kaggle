...
# 填补 FireplaceQu 缺失值
# init_f = ['MSSubClass','MSZoning','Utilities','OverallQual','OverallCond','YearBuilt','Heating','HeatingQC','Electrical','Fireplaces']
#
# X = all_data[all_data.FireplaceQu.notnull()][init_f]
# y = all_data[all_data.FireplaceQu.notnull()]['FireplaceQu']
# X_dumm = pd.get_dummies(X)
#
# a = selector.fit(X_dumm, y)
#
# sorted_index = np.argsort(a.scores_)[-16:]
# selected_columns = X_dumm.columns[sorted_index]
#
# X = all_data[all_data.FireplaceQu.notnull()]
# X = pd.get_dummies(X)
# X = X[selected_columns]
# y = all_data[all_data.FireplaceQu.notnull()]['FireplaceQu']
#
# X_test = all_data[all_data.FireplaceQu.isnull()][init_f]
# ic(X_test)
...
# # ])
# # param_grid = {
# #     'rfc__n_estimators': list(range(2, 60, 2)),
# #     'rfc__max_depth': list(range(2, 20, 2))
# # }
# # gsearch = GridSearchCV(estimator=pipe, param_grid=param_grid, cv=5, n_jobs=-1, scoring='accuracy')
# # gsearch.fit(X, y)
# # ic(gsearch.best_params_, gsearch.best_score_)
#
# '''评估模型'''
# pipe_best = Pipeline([
#     ('rfc', RandomForestClassifier(random_state=6, max_depth=6, n_estimators=14))
# ])
# # scores = cross_val_score(estimator=pipe_best, X=X, y=y, cv=5, scoring='accuracy')
# # ic(np.mean(scores), np.std(scores))
# pipe_best.fit(X, y)
# y_pred = pipe_best.predict(X_test)
# ic(type(y_pred), len(y_pred))
# ic(all_data[all_data.FireplaceQu.isnull()])
# # all_data.loc[all_data.FireplaceQu.isnull(), 'FireplaceQu'] = y_pred
# # ic(all_data[all_data.FireplaceQu.isnull()])
...
all_data = all_data[high_features]
...
res = selector.fit(X_dumm, y)

sorted_index = np.argsort(-res.scores_)[:11]

'''绘图'''
# plt.style.use({'figure.figsize': (30, 12)})
# sns.barplot(y=X_dumm.columns[sorted_index], x=np.sort(res.scores_)[::-1][:11], orient='h')
# plt.show()
...
# X_train = X_dumm[selected_columns]
X_train = X_dumm

'''预测数据'''
X_test = test_data.drop('SalePrice', axis=1)
X_test_dumm = pd.get_dummies(X_test)
# X_test_dumm = X_test_dumm[selected_columns]
...
'''评估模型'''
scores = cross_val_score(estimator=model, X=X_train, y=y, cv=5, scoring='r2', n_jobs=-1)
ic(np.mean(scores), np.std(scores))
exit()
