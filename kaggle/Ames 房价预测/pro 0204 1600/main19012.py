...
pca.fit(X_train)
ic(pca.explained_variance_ratio_, pca.explained_variance_)
X_train_reduction = pca.transform(X_train)
X_test_dumm_reduction = pca.transform(X_test_dumm)
ic(X_train_reduction.shape)
ic(X_test_dumm_reduction.shape)
...
'''评估模型'''
# scores = cross_val_score(estimator=model, X=X_train_reduction, y=y, cv=5, scoring='neg_mean_squared_error', n_jobs=-1)
# ic(np.mean(scores), np.std(scores))

'''训练'''
model.fit(X_train_reduction, y)
y_pred = model.predict(X_test_dumm_reduction)
